...
namespace nSCGL
{
	...
	uint32_t cGDriver::NewBufferRegion(int32_t gdBufferRegionType) {
		...
		HANDLE result = wglCreateBufferRegionARB(reinterpret_cast<HDC>(deviceContext), 1, regionTypeMap[gdBufferRegionType]);
		if (result == nullptr) {
			return 0;
		}
	...
	bool cGDriver::ReadBufferRegion(uint32_t region, GLint x, GLint y, GLsizei width, GLsizei height, int32_t unused0, int32_t unused1) {
		bool result = wglSaveBufferRegionARB(bufferRegionHandles[region - 1], x, y, width, height);
		return result;
	}

	bool cGDriver::DrawBufferRegion(uint32_t region, GLint x, GLint y, GLsizei width, GLsizei height, GLint xDest, GLint yDest) {
		bool result = wglRestoreBufferRegionARB(bufferRegionHandles[region - 1], xDest, yDest, width, height, x, y);
		return result;
	}
